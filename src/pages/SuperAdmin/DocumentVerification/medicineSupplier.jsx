...
const MedicineSupplierDocuments = () => {
  const [searchTerm, setSearchTerm] = useState('')
  const [selectedStatus, setSelectedStatus] = useState('all')

  // Mock data for medicine supplier documents
  const documents = [
    {
      id: 1,
    ...
    },
    {
      ...
      fileSize: '1.2 MB',
      uploadedBy: 'Head Pharmacist',
    },
    {
      id: 7,
      name: 'Storage Facility Compliance',
      type: 'Compliance Document',
      status: 'verified',
  ...
  return (
    <div className='p-6'>
      <div className='mb-6'>
        <h2 className='text-xl font-semibold text-slate-800 mb-2'>Medicine Supplier Documents</h2>
        <p className='text-slate-600 text-sm'>
          List of compliance and legal documents required for medicine supplier onboarding process
        </p>
      </div>
          ...
          <tbody>
            {filteredDocuments.map((doc) => (
              <tr key={doc.id} className='border-b border-slate-100 hover:bg-slate-50'>
                ...
                <td className='py-3 px-4'>
                  <span
                    className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(doc.status)}`}
                  >
...
export default MedicineSupplierDocuments
