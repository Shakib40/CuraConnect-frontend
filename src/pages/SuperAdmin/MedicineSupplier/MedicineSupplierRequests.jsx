...
const mockRequests = [
    {
        ...
        message: "Global Med Distributors seeks to integrate with CuraConnect to supply over 5,000 pharmaceutical SKUs to registered hospitals across North America.",
        signedDocs: [
    ...
    },
    {
        ...
        state: "IL",
        country: "United States",
        message: "Swift Pharma Logistics specialises in temperature-controlled pharmaceutical delivery and is seeking to become a logistics partner for CuraConnect's hospital network.",
        signedDocs: [
    ...
    },
    {
        ...
        message: "BioHealth Pure manufactures FDA and MHRA approved biologics and seeks to supply CuraConnect-affiliated hospitals with specialty pharmaceutical products.",
        signedDocs: [
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
        ...
        <div className="relative w-full max-w-md h-full bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider ${request.license === "Verified" ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"
                                }`}>
                                <div className={`w-1.5 h-1.5 rounded-full ${request.license === "Verified" ? "bg-green-500" : "bg-amber-400"}`} />
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
                        ...
                        {request.signedDocs.map((doc, idx) => (
                            <div key={idx} className={`flex items-start gap-3 p-3 rounded-xl border ${doc.signed ? "bg-green-50 border-green-100" : "bg-slate-50 border-slate-200"
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className={`text-xs font-bold leading-snug ${doc.signed ? "text-green-800" : "text-slate-500"}`}>{doc.name}</p>
                                    {doc.signed
                ...
                {request.license === "Pending" && (
                    <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 flex gap-2">
                        ...
                        <p className="text-xs text-amber-800 font-medium">License is still pending verification. Ensure GMP certificates are valid before approving.</p>
                    </div>
                )}
            </div>
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex gap-3">
                ...
                <Button variant="success" icon={CheckCircle2} className="flex-1" onClick={() => onApprove(request)}>
                    Approve
                </Button>
            </div>
        </div>
