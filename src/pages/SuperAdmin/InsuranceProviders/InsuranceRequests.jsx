...
const mockRequests = [
  {
    ...
    message:
      "Global Health Insurance is seeking to expand its network to include CuraConnect's hospital base across Europe and South Asia.",
    signedDocs: [
  ...
  },
  {
    ...
    message:
      'United Assurance aims to partner with CuraConnect to provide affordable health plans to patients in the Midwest region.',
    signedDocs: [
  ...
  },
  {
    ...
    country: 'Australia',
    message:
      "Prime Care specialises in chronic disease management plans and is looking to integrate with CuraConnect's diagnostic network.",
    signedDocs: [
        ...
        <div className='flex items-center gap-3'>
          ...
          <div>
            ...
            <div className='flex items-center gap-1.5 mt-0.5'>
              ...
              >
                <div
                  className={`w-1.5 h-1.5 rounded-full ${request.docs === 'Verified' ? 'bg-green-500' : 'bg-amber-400'}`}
                />
                Docs {request.docs}
              </span>
                ...
                <div className='flex-1 min-w-0'>
                  <p
                    className={`text-xs font-bold leading-snug ${
                      doc.signed ? 'text-green-800' : 'text-slate-500'
                    }`}
                  >
                    {doc.name}
                  </p>
        ...
        {request.docs === 'Pending' && (
          <div className='p-3 bg-amber-50 rounded-xl border border-amber-200 flex gap-2'>
            ...
            <p className='text-xs text-amber-800 font-medium'>
              Documents are still pending verification. Review before approving.
            </p>
          </div>
        )}
      </div>
...
const InsuranceRequests = () => {
  ...
  const columns = [
    ...
    { header: 'Region', accessor: 'region' },
    {
      ...
      render: (row) => (
        <div
          className={`flex items-center gap-1.5 font-bold text-[11px] ${row.docs === 'Verified' ? 'text-success' : 'text-warning'}`}
        >
      ...
      <div className='p-5 bg-blue-50/50 rounded-2xl border border-dashed border-blue-200 flex gap-4'>
        ...
        <div>
          ...
          <p className='text-xs text-blue-700 mt-1 leading-relaxed max-w-2xl'>
            Verify the provider's regulatory license status and regional coverage before approval.
            Approved providers will be immediately notified via email to complete their profile
            setup.
          </p>
        </div>
      </div>
