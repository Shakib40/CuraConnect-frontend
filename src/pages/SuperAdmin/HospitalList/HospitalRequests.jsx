...
import {
    ...
    XCircle, X, ChevronRight, AlertCircle, TrendingUp
} from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
        ...
        <div className="relative w-full max-w-md h-full bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-medium">{request.type} · {request.location}</p>
                    </div>
                </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
                        ...
                        {request.signedDocs.map((doc, idx) => (
                            <div key={idx} className={`flex items-start gap-3 p-3 rounded-xl border ${doc.signed ? "bg-green-50 border-green-100" : "bg-slate-50 border-slate-200"
                                ...
                                <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${doc.signed ? "bg-green-100" : "bg-slate-200"
                                    ...
                                    {doc.signed ? <CheckCircle2 className="w-4 h-4 text-green-600" /> : <XCircle className="w-4 h-4 text-slate-400" />}
                                </div>
                ...
                {request.signedDocs.some((d) => !d.signed) && (
                    <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 flex gap-2">
                        ...
                        <p className="text-xs text-amber-800 font-medium">Some documents are still pending signature. Review before approving.</p>
                    </div>
                )}
            </div>
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex gap-3">
                <Button variant="outline" icon={XCircle}
...
const HospitalRequests = () => {
    ...
    const columns = [
        ...
        { header: "Request Date", accessor: "date" },
        {
            header: "Action",
            render: (row) => (
                <button onClick={() => setSelectedRequest(row)}
        ...
        <div className="p-0">
            <div className="p-4 bg-teal-50/50 flex items-center gap-3 border-b border-teal-100">
                ...
                <p className="text-sm font-medium text-teal-800">
                    Review pending registration requests from hospitals wanting to join the platform. ({requests.length} pending)
                </p>
            </div>

            <Table columns={columns} data={requests} currentPage={1} totalPages={1} onPageChange={() => { }} />
