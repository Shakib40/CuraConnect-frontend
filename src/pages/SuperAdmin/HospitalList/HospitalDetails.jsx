    ...
    <div className='bg-white'>
      ...
      <div className='p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4'>
        <div className='flex items-center gap-4'>
          ...
          <div>
            ...
            <div className='flex items-center gap-2 mt-1 flex-wrap'>
              ...
              >
                <div
                  className={`w-1.5 h-1.5 rounded-full ${hospital.status === 'Active' ? 'bg-green-500' : 'bg-slate-400'}`}
                />
                {hospital.status}
              </span>
              <span
                className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${planColors[hospital.plan]}`}
              >
                {hospital.plan}
              </span>
            </div>
        ...
        <div className='flex items-center gap-3'>
          <button
            onClick={() => navigate('/superadmin/hospitals/list')}
          ...
          <Button
            icon={Pencil}
            onClick={() => navigate(`/superadmin/hospitals/${hospital.id}/edit`)}
          >
            Edit Hospital
          </Button>
        </div>
      ...
      <div className='p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-2 gap-6'>
        ...
        <div className='space-y-4 p-6 bg-slate-50 rounded-2xl border border-slate-100'>
          ...
          <InfoRow
            icon={Building2}
            label='Hospital Type'
            value={hospital.type === 'private' ? 'Private' : 'Government'}
          />
          ...
          <InfoRow
            icon={Building2}
            label='Owner'
            value={`${hospital.ownerFirstName} ${hospital.ownerLastName}`}
          />
        </div>
        ...
        <div className='space-y-4 p-6 bg-slate-50 rounded-2xl border border-slate-100'>
          ...
          <div className='space-y-2'>
            {complianceDocs.map((doc) => (
              ...
              >
                ...
                <span
                  className={`text-sm font-medium ${hospital[doc.id] ? 'text-slate-800' : 'text-slate-400'}`}
                >
                  {doc.label}
                </span>
          ...
          <div className='p-3 bg-blue-50 rounded-xl border border-blue-100 flex gap-2'>
            ...
            <p className='text-[11px] text-blue-800 font-medium'>
              Checked documents are shared with this hospital's dashboard.
            </p>
          </div>
        </div>
      </div>
