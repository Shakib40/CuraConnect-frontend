...
const HospitalList = () => {
        ...
        { header: "Joined On", accessor: "joinedDate" },
        {
            ...
            render: (row) => (
                <div className="flex items-center gap-2 flex-wrap">
                    <button onClick={() => navigate(`/superadmin/hospitals/${row.id}`)}
                    ...
                    <button onClick={() => navigate(`/superadmin/hospitals/${row.id}/edit`)}
                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors">
                        <Pencil className="w-3.5 h-3.5" /> Edit
                    </button>
                    <button onClick={() => setModal({ hospital: row, isDeactivating: row.status === "Active" })}
    ...
    return (
        <div className="p-0">
            ...
            <Table columns={columns} data={hospitals} currentPage={currentPage} totalPages={1} onPageChange={setCurrentPage} />
