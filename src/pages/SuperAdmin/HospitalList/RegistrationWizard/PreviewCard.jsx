  ...
  return (
    <div className='bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200 overflow-hidden border border-slate-100 transform rotate-1 hover:rotate-0 transition-transform duration-500'>
      ...
      <div className='px-6 pb-8 -mt-10 relative'>
        ...
        <div className='mt-6 space-y-4'>
          <div className='flex flex-wrap gap-1.5'>
            ...
            {values.specialties?.slice(0, 2).map((s, i) => (
              <span
                key={i}
                className='px-2 py-0.5 bg-slate-100 text-[10px] font-bold text-slate-600 rounded-full border border-slate-200 uppercase'
              >
                {s.label}
              </span>
            ))}
        ...
        <button
          type='button'
          className='w-full mt-6 py-4 bg-teal-600 text-white text-sm font-bold rounded-2xl shadow-lg shadow-teal-600/30 active:scale-95 transition-all'
        >
          Book Appointment
        </button>
      </div>
