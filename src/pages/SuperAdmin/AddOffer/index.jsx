...
import { ArrowLeft, Plus, Save, X, Users, Building2, Truck, Gift, Percent, Calendar, Tag, Target, Package, CheckCircle, AlertTriangle, Info } from "lucide-react";
...
const AddOfferPage = () => {
    ...
    const offerTypeOptions = [
        { value: "discount", label: "Discount", icon: <Percent className="w-4 h-4 text-green-500" />, description: "Percentage-based discount" },
        { value: "free_service", label: "Free Service", icon: <Gift className="w-4 h-4 text-blue-500" />, description: "Complimentary service offering" },
        { value: "coupon", label: "Coupon", icon: <Tag className="w-4 h-4 text-purple-500" />, description: "Promotional coupon code" }
    ];
        ...
        patient: [
            {
                ...
                description: "Special discount on annual health checkup packages including blood work and consultation",
                type: "discount",
                discountValue: "20",
    ...
    const validateForm = () => {
        ...
        if (!formData.usageLimit || formData.usageLimit <= 0) newErrors.usageLimit = "Usage limit must be greater than 0";
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 font-medium text-sm mt-1">Create a new discount or special offer</p>
                    </div>
                </div>
            </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="bg-white p-6 rounded-lg border border-slate-200">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-2">Entity ID (Optional)</label>
                                    <input
                                        type="text"
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-slate-700 mb-2">
                                            {formData.type === "free_service" ? "Value (100 for Free)" : "Discount Value (%)"}
                                        </label>
                                        <input
                                            type="number"
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-lg border border-slate-200">
                        ...
                        <div className="space-y-3">
                            {offerTypeOptions.map((option) => (
                                <div key={option.value} className="flex items-start gap-3 p-3 rounded-lg bg-slate-50">
                                    {option.icon}
                                    <div>
                        ...
                        <div className="bg-white p-6 rounded-lg border border-slate-200">
                            ...
                            <div className="space-y-3">
                                {currentSuggestedOffers.map((suggestion, index) => (
                                    <div key={index} className="p-3 border border-slate-200 rounded-lg hover:border-primary hover:bg-primary/5 transition-all cursor-pointer"
                                         onClick={() => applySuggestedOffer(suggestion)}>
                                        <h4 className="font-medium text-slate-800 text-sm">{suggestion.title}</h4>
                                        <p className="text-xs text-slate-600 mt-1 line-clamp-2">{suggestion.description}</p>
                                        <div className="flex items-center gap-2 mt-2">
                                            <span className="text-xs px-2 py-1 bg-primary/10 text-primary rounded-full">
                                                {suggestion.type}
                                            </span>
                                            <span className="text-xs text-slate-500">
                                                {suggestion.discountValue}%{suggestion.type === 'free_service' ? ' (FREE)' : ''}
                                            </span>
                                        </div>
                                    </div>
                                ))}
