  ...
  const renderBarChart = () => (
    <div className='bg-white p-6 rounded-lg border border-slate-200'>
      <h3 className='text-lg font-semibold text-slate-800 mb-4'>
        Monthly Patient Registration & Appointments
      </h3>
    ...
    <div className='bg-white p-6 rounded-lg border border-slate-200'>
      ...
      <div className='grid grid-cols-3 gap-4 mt-4 pt-4 border-t border-slate-100'>
        ...
        <div className='text-center'>
          <div className='text-lg font-semibold text-slate-800'>
            {Math.round(
              monthlyData.reduce((sum, d) => sum + d.appointments, 0) / monthlyData.length,
            )}
          </div>
        ...
        <div className='text-center'>
          <div className='text-lg font-semibold text-green-600'>
            +
            {Math.round(
              ((monthlyData[monthlyData.length - 1].appointments - monthlyData[0].appointments) /
                monthlyData[0].appointments) *
                100,
            )}
            %
          </div>
            ...
            >
              ...
              <div className='text-right'>
                ...
                <span className='text-xs text-slate-600 block'>
                  {Math.round(
                    (item.value / 100) * monthlyData.reduce((sum, d) => sum + d.appointments, 0),
                  )}{' '}
                  appts
                </span>
              </div>
        ...
        <div className='relative h-full flex items-end justify-between gap-3 ml-8 pb-8'>
          {monthlyData.map((item, index) => (
            <div key={index} className='flex-1 flex flex-col items-center justify-end relative'>
              ...
              <span className='text-xs text-slate-600 mt-2 font-medium absolute bottom-0'>
                {item.month}
              </span>
            </div>
          ))}
      ...
      <div className='space-y-4'>
        {patients.slice(0, 5).map((patient) => (
          ...
          >
            ...
            <div className='flex items-center gap-2'>
              <div className='flex'>{renderStars(patient.feedbackRating)}</div>
  ...
  return (
    <div className='space-y-6'>
      <div className='flex flex-col md:flex-row md:items-center justify-between gap-4'>
        <div>
          ...
          <p className='text-slate-500 font-medium text-sm mt-1'>
            Manage patients and view analytics.
          </p>
        </div>
      </div>
      ...
      <div className='bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden'>
                ...
                <tbody>
                  {filteredPatients.map((patient) => (
                    <tr key={patient.id} className='border-b border-slate-100 hover:bg-slate-50'>
                      ...
                      <td className='py-3 px-4'>
                        <span
                          className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(patient.status)}`}
                        >
        ...
        {activeTab === 'feedback' && <div className='p-6'>{renderFeedbackSection()}</div>}
      </div>
    </div>
  )
}
