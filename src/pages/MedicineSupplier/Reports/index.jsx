...
const ReportsPage = () => {
  ...
  const headers = [
    ...
    },
    {
      ...
      render: (report) => (
        <span
          className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getTypeColor(report.type)}`}
        >
          {report.type}
        </span>
      ),
    },
    {
      header: 'Period',
      accessor: 'period',
      render: (report) => <span className='text-sm text-slate-900'>{report.period}</span>,
    },
    {
      header: 'Generated',
    ...
    },
    {
      ...
      render: (report) => (
        <div className='text-sm text-slate-900'>
          ...
          {report.growthRate && (
            <div
              className={`flex items-center gap-1 ${report.growthRate > 0 ? 'text-green-600' : 'text-red-600'}`}
            >
              {report.growthRate > 0 ? (
  ...
  return (
    <div className='space-y-6'>
      ...
      <Formik initialValues={formValues} onSubmit={handleFormChange} enableReinitialize>
        {() => (
          <Form>
            <div className='bg-white p-4 rounded-lg border border-slate-200'>
              <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                ...
                <div className='md:col-span-1'>
                  <Select name='dateRange' label='Date Range' options={options?.dateRanges} />
                </div>
              </div>
            </div>
          </Form>
      ...
      <Table columns={headers} data={filteredReports} />
          ...
          {() => (
            <Form>
              <div className='space-y-4'>
                ...
                <div>
                  <Input name='toDate' type='date' label='To Date' placeholder='Select end date' />
                </div>
              </div>
            </Form>
          )}
