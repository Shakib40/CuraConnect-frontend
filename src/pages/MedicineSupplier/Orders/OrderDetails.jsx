...
const OrderDetails = () => {
  ...
  const orderItemsColumns = [
    ...
    },
    {
      header: 'Quantity',
      accessor: 'quantity',
      render: (item) => <span className='text-sm font-medium text-slate-900'>{item.quantity}</span>,
    },
    {
      header: 'Unit Price',
      accessor: 'unitPrice',
    ...
    },
    {
      ...
      render: (item) => <span className='text-sm text-slate-600'>{item.prescribedBy}</span>,
    },
    {
      header: 'Instructions',
      accessor: 'instructions',
      render: (item) => <span className='text-sm text-slate-600'>{item.instructions}</span>,
    },
  ]
  ...
  const orderHistoryColumns = [
    ...
    },
    {
      header: 'Note',
      accessor: 'note',
      render: (history) => <span className='text-sm text-slate-600'>{history.note}</span>,
    },
    {
      header: 'Updated By',
      accessor: 'updatedBy',
      render: (history) => <span className='text-sm text-slate-600'>{history.updatedBy}</span>,
    },
  ]
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='flex items-center justify-between mb-6'>
        ...
        <div className='flex items-center gap-4'>
          ...
          <div className='flex items-center gap-2'>
            <span
              className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(currentOrder.status)}`}
            >
              {currentOrder.status}
            </span>
            ...
            <div className='flex justify-between'>
              ...
              <span className='text-sm text-slate-600'>
                {currentOrder.actualDelivery
                  ? new Date(currentOrder.actualDelivery).toLocaleDateString()
                  : 'Not delivered'}
              </span>
            </div>
          </div>
        ...
        <div className='bg-white p-6 rounded-lg border border-slate-200'>
          ...
          <div className='space-y-3'>
            ...
            <div className='flex justify-between'>
              ...
              <span className='text-sm text-slate-600'>
                ${currentOrder.shippingCharges.toFixed(2)}
              </span>
            </div>
        ...
        <div className='bg-white p-6 rounded-lg border border-slate-200'>
          ...
          <div className='space-y-3'>
            ...
            <div className='flex justify-between'>
              ...
              <span className='text-sm text-slate-600'>
                {currentOrder.paymentDetails?.authorizationCode || 'N/A'}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className='bg-white p-6 rounded-lg border border-slate-200'>
        ...
        <Table columns={orderItemsColumns} data={currentOrder.items} className='mt-4' />
      </div>

      {/* Order History */}
      <div className='bg-white p-6 rounded-lg border border-slate-200'>
        <h2 className='text-lg font-semibold text-slate-800 mb-4'>Order History</h2>
        <Table columns={orderHistoryColumns} data={currentOrder.history} className='mt-4' />
      </div>
