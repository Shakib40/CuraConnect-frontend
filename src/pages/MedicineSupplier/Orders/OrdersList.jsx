...
const OrdersList = () => {
  ...
  const statuses = [{ value: 'all', label: 'All Status' }, ...options.orderStatus]
  ...
  const filteredOrders = useMemo(() => {
    return ordersData.filter((order) => {
      ...
      const matchesHospital =
        filterValues.hospital === 'all' || order.hospitalName === filterValues.hospital
  ...
  return (
    <div className='space-y-6'>
      ...
      {/* Filters */}
      <Formik initialValues={filterValues} onSubmit={updateFilterValues} enableReinitialize>
        {({ values, submitForm }) => {
            ...
            <Form onChange={handleFieldChange}>
              <div className='bg-white p-4 rounded-lg border border-slate-200'>
                <div className='grid grid-cols-1 md:grid-cols-4 gap-4'>
                  ...
                  <Select name='hospital' label='Hospital' options={hospitals} />
                  <Select name='dateRange' label='Date Range' options={dateRanges} />
                </div>
      ...
      <Table
        columns={[
          ...
          },
          {
            ...
            render: (order) => (
              <span
                className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getPriorityColor(order.priority)}`}
              >
                {order.priority}
              </span>
            ),
          },
          {
            ...
            render: (order) => (
              <div className='flex items-center gap-2'>
                ...
                <span
                  className={`text-sm font-medium ${getStatusColor(order.status).split(' ')[0]}`}
                >
                  {order.status}
                </span>
              </div>
          ...
          {() => (
            <Form>
              <div className='space-y-4'>
                <Select name='status' label='New Status' options={options.orderStatus} />
                <Input
                  name='notes'
                  label='Update Notes'
                  type='textarea'
