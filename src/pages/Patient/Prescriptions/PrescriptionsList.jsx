...
const PrescriptionsList = () => {
  ...
  const doctorColumns = [
    ...
    {
      header: 'Total Cost',
      render: (row) => <span className='font-semibold text-slate-800'>{row.totalCost}</span>,
    },
    {
      header: 'Last Visit',
      render: (row) => (
  ...
  const medicineColumns = [
      ...
      render: (row) => (
        <span
          className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${
            row.refills > 0 ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-800'
          }`}
        >
          {row.refills > 0 ? <CheckCircle className='w-3 h-3' /> : <Clock className='w-3 h-3' />}
    ...
    {
      header: 'Cost',
      render: (row) => <span className='font-semibold text-slate-800'>{row.payment.amount}</span>,
    },
    {
      header: 'Action',
      render: (row) => (
        ...
        <div className='flex items-center gap-4 mb-6'>
          ...
          <div>
            ...
            <p className='text-slate-500'>
              {selectedDoctor.specialty} • {selectedDoctor.totalPrescriptions} Prescriptions
            </p>
          </div>
        </div>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6'>
            <div className='bg-white rounded-lg p-4'>
              ...
              <p className='text-2xl font-bold text-slate-800'>
                $
                {selectedDoctor.medicines
                  .reduce((sum, med) => sum + parseFloat(med.payment.amount.replace('$', '')), 0)
                  .toFixed(2)}
              </p>
            </div>
            <div className='bg-white rounded-lg p-4'>
              ...
              <p className='text-2xl font-bold text-slate-800'>
                $
                {(
                  selectedDoctor.medicines.reduce(
                    (sum, med) => sum + parseFloat(med.payment.amount.replace('$', '')),
                    0,
                  ) * 0.18
                ).toFixed(2)}
              </p>
              ...
              <div className='space-y-2'>
                <div className='flex justify-between items-center'>
                  ...
                  <span className='font-medium text-slate-800'>
                    $
                    {selectedDoctor.medicines
                      .filter((med) => med.payment.method === 'Insurance')
                      .reduce(
                        (sum, med) => sum + parseFloat(med.payment.amount.replace('$', '')),
                        0,
                      )
                ...
                <div className='flex justify-between items-center'>
                  ...
                  <span className='font-medium text-slate-800'>
                    $
                    {selectedDoctor.medicines
                      .filter((med) => med.payment.method === 'Credit Card')
                      .reduce(
                        (sum, med) => sum + parseFloat(med.payment.amount.replace('$', '')),
                        0,
                      )
                ...
                <div className='flex justify-between items-center'>
                  ...
                  <span className='font-medium text-slate-800'>
                    $
                    {selectedDoctor.medicines
                      .filter((med) => med.payment.method === 'Cash')
                      .reduce(
                        (sum, med) => sum + parseFloat(med.payment.amount.replace('$', '')),
                        0,
                      )
              ...
              <div className='space-y-2'>
                <div className='flex justify-between items-center'>
                  ...
                  <span className='font-medium text-green-800'>
                    $
                    {selectedDoctor.medicines
                      .filter((med) => med.payment.status === 'Paid')
                      .reduce(
                        (sum, med) => sum + parseFloat(med.payment.amount.replace('$', '')),
                        0,
                      )
                ...
                <div className='flex justify-between items-center'>
                  ...
                  <span className='font-medium text-amber-600'>
                    $
                    {selectedDoctor.medicines
                      .filter((med) => med.payment.status === 'Pending')
                      .reduce(
                        (sum, med) => sum + parseFloat(med.payment.amount.replace('$', '')),
                        0,
                      )
                ...
                <div className='flex justify-between items-center'>
                  ...
                  <span className='font-medium text-red-600'>
                    $
                    {selectedDoctor.medicines
                      .filter((med) => med.payment.status === 'Failed')
                      .reduce(
                        (sum, med) => sum + parseFloat(med.payment.amount.replace('$', '')),
                        0,
                      )
