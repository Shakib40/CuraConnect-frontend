...
const Insurance = () => {
  ...
  const handleFormSubmit = (formData) => {
    if (editingInsurance) {
      setInsurances(
        insurances.map((ins) =>
          ins.id === editingInsurance.id ? { ...formData, id: editingInsurance.id } : ins,
        ),
      )
    } else {
    ...
    <div className='space-y-6'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
        {insurances.map((insurance) => (
          <div
            key={insurance.id}
            className={`bg-white rounded-lg border ${insurance.isPrimary ? 'border-teal-500' : 'border-slate-200'} p-6`}
          >
