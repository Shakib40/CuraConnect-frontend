...
const OnboardingReview = () => {
    const [documentStatus, setDocumentStatus] = useState({
        aadhar: { status: 'verified', uploadedAt: '2024-02-20T10:30:00Z', verifiedAt: '2024-02-20T14:45:00Z' },
        photo: { status: 'pending', uploadedAt: '2024-02-20T10:35:00Z', verifiedAt: null },
        panCard: { status: 'verified', uploadedAt: '2024-02-20T10:40:00Z', verifiedAt: '2024-02-20T15:20:00Z' },
        ...
        pharmacyCertificate: { status: 'rejected', uploadedAt: '2024-02-19T16:20:00Z', verifiedAt: null, rejectionReason: 'Document is unclear or expired' }
    });
    ...
    const getStatusColor = (status) => {
        switch (status) {
            ...
            case 'not_uploaded': return 'text-slate-600 bg-slate-50 border-slate-200';
    ...
    const getDocumentIcon = (docType) => {
        switch (docType) {
            case 'aadhar': return <FileText className="w-6 h-6" />;
            case 'photo': return <Camera className="w-6 h-6" />;
            case 'panCard': return <CreditCard className="w-6 h-6" />;
            case 'bankDetails': return <FileText className="w-6 h-6" />;
            case 'gstNumber': return <Shield className="w-6 h-6" />;
            case 'medicalLicense': return <Shield className="w-6 h-6" />;
            case 'pharmacyCertificate': return <FileText className="w-6 h-6" />;
    ...
    const getDocumentName = (docType) => {
        switch (docType) {
            ...
            case 'gstNumber': return 'GST Number';
            case 'medicalLicense': return 'Medical License';
            case 'pharmacyCertificate': return 'Pharmacy Certificate';
    ...
    const overallStatus = {
        ...
        notUploaded: Object.values(documentStatus).filter(doc => doc.status === 'not_uploaded').length
    };
                ...
                <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
                    <div className="flex items-center justify-between mb-4">
                        <div>
                            ...
                            <p className="text-slate-600 mt-1">Track the verification progress of your submitted documents</p>
                        </div>
                        <button
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-2">
                                        {status.status !== 'not_uploaded' && (
                                            <>
                                                <button className="p-2 text-slate-600 hover:text-teal-600 transition-colors" title="View Document">
                                                    <Eye className="w-4 h-4" />
                                                </button>
                                                <button className="p-2 text-slate-600 hover:text-teal-600 transition-colors" title="Download">
